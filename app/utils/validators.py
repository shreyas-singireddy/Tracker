...
def calculate_age(birth_date_str: str) -> int:
    ...
    try:
        ...
        raise ValidationError(
            message="Invalid birth date format. Must be YYYY-MM-DD.",
            details=f"Received: {birth_date_str}"
        )
...
def validate_user_profile(birth_date: str, weight_kg: float, height_cm: float):
    ...
    if weight_kg <= 0:
        raise ValidationError(
            message="User validation failed: Weight must be greater than 0 kg.",
            details=f"Received weight: {weight_kg}"
        )
...
def validate_food_item(calories: float, protein: float, carbs: float, fats: float):
    ...
    if calories < 0:
        raise ValidationError(
            message="Food validation failed: Calories cannot be negative.",
            details=f"Calories: {calories}"
        )
...
def validate_timestamp(logged_at_str: str):
    ...
    for fmt in ("%Y-%m-%d %H:%M:%S", "%Y-%m-%dT%H:%M:%S", "%Y-%m-%d %H:%M:%S.%f", "%Y-%m-%dT%H:%M:%S.%f"):
        try:
            ...
            clean_str = logged_at_str.split("+")[0].split("Z")[0]
...
def validate_email(email: str):
    ...
    if not re.match(email_regex, email):
        raise ValidationError(
            message="Validation failed: Email address is invalid.",
            details=f"Received: {email}"
        )
...
def validate_habit_name(name: str):
    ...
    if not name or not name.strip():
        raise ValidationError(
            message="Habit validation failed: Name cannot be empty.",
            details=f"Received: {name}"
        )
...
def validate_habit_frequency(frequency: str):
    ...
    if frequency not in valid:
        raise ValidationError(
            message="Habit validation failed: Frequency must be 'daily' or 'weekly'.",
            details=f"Received: {frequency}"
        )
...
def validate_habit_target_value(target_value: float):
    ...
    if target_value <= 0:
        raise ValidationError(
            message="Habit validation failed: Target value must be greater than 0.",
            details=f"Received: {target_value}"
        )
...
def validate_sleep_hours(hours: float):
    ...
    if not (0 <= hours <= 24):
        raise ValidationError(
            message="Sleep validation failed: Hours must be between 0 and 24.",
            details=f"Received: {hours}"
        )
...
def validate_recovery_score(score: float):
    ...
    if not (0 <= score <= 100):
        raise ValidationError(
            message="Recovery validation failed: Score must be between 0 and 100.",
            details=f"Received: {score}"
        )

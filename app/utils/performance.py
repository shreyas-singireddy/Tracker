    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if elapsed > threshold:
                logger.warning(
                    f"PERF: {func.__module__}.{func.__qualname__} took {elapsed:.3f}s (threshold: {threshold}s)"
                )
            else:
                logger.debug(f"PERF: {func.__module__}.{func.__qualname__} took {elapsed:.3f}s")
            return result
...
def batch_read(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        if isinstance(result, list) and len(result) > 10:
            logger.debug(f"BATCH: {func.__module__}.{func.__qualname__} returned {len(result)} rows")
        return result

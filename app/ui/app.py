...
def run_app():
    ...
    st.sidebar.markdown("<p style='font-size: 0.8rem; font-weight: 600; color: #94a3b8; margin-bottom: 8px;'>SYSTEM HEALTH</p>", unsafe_allow_html=True)
    if settings.OFFLINE_MODE:
        st.sidebar.markdown("<span style='color: #10B981; font-size: 0.85rem;'>🔒 Offline Mode: Enforced</span>", unsafe_allow_html=True)
    else:
        st.sidebar.markdown("<span style='color: #EF4444; font-size: 0.85rem;'>⚠️ Offline Mode: Compromised</span>", unsafe_allow_html=True)

    if db_ok:
        st.sidebar.markdown("<span style='color: #10B981; font-size: 0.85rem;'>💾 Database: Connected</span>", unsafe_allow_html=True)
    else:
        st.sidebar.markdown("<span style='color: #EF4444; font-size: 0.85rem;'>❌ Database: Disconnected</span>", unsafe_allow_html=True)
    ...
    with main_container:
        ...
        try:
            ...
            st.error(f"Failed to load page '{selection}': {str(e)}")

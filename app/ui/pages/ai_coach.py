...
def render():
    ...
    if sessions:
        selected_sid = st.sidebar.selectbox(
            "Select Session Thread", options=list(session_options.keys()), format_func=lambda x: session_options[x]
        )
    ...
    with st.form(key="chat_input_form", clear_on_submit=True):
        user_query = st.text_input(
            "Ask about workout plans, protein goals, recovery status, or sleep logs:",
            placeholder="Why am I tired today?",
        )

...
def render():
        ...
        if active_session:
                    ...
                    with st.expander(f"💪 {ex_name}", expanded=True):
                        ...
                        if sets:
                            for s in sorted(sets, key=lambda x: x.set_number):
                                st.write(
                                    f"Set {s.set_number}: **{s.weight:.1f} kg** × **{s.reps} reps** (RPE: {s.rpe or 'N/A'})"
                                )
                        else:
                        ...
                        with st.form(key=f"set_form_{elog.exercise_log_id}"):
                            ...
                            weight = col_w.number_input(
                                "Weight (kg)", min_value=0.0, step=2.5, key=f"w_{elog.exercise_log_id}"
                            )
                            ...
                            rpe = col_rp.slider(
                                ...
                                value=8.0,
                                step=0.5,
                                key=f"rp_{elog.exercise_log_id}",
                            )
                            ...
                            if submit_set:
                                try:
                                    ...
                                    workout_service.log_set(
                                        set_id, session_id, elog.exercise_log_id, set_num, weight, reps, rpe
                                    )
            ...
            if exercises:
                ...
                selected_ex_id = st.selectbox(
                    "Select Exercise", options=list(ex_options.keys()), format_func=lambda x: ex_options[x]
                )
                ...
                with st.expander(f"📅 Workout on {s.start_time} — {s.calories_burned_kcal or 0:.0f} kcal"):
                    ...
                    if elogs:
                        ...
                        for elog in elogs:
                            ...
                            sets = workout_service.set_repo.get_log_sets(elog.exercise_log_id)
                            sets_str = ", ".join(
                                [f"{s.weight:.1f}kg×{s.reps}" for s in sorted(sets, key=lambda x: x.set_number)]
                            )
    ...
    with tab_catalog:
        ...
        with st.form(key="add_ex_form"):
            ...
            form_rules = st.text_area(
                'Form Rules (JSON dict, e.g. {"setup": "Feet flat", "execution": "Lower to chest"})',
                value='{"setup": "", "execution": ""}',
            )

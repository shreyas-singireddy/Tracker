...
class GoalService:
    ...
    def __init__(self, goal_repo: Optional[GoalRepository] = None, user_repo: Optional[UserRepository] = None):
    ...
    def add_goal(self, goal: Goal) -> str:
        ...
        if not self.user_repo.get_user(goal.user_id):
            ...
            raise ValidationError(
                message="Goal validation failed: User account not found.",
                details=f"User ID: {goal.user_id}"
            )

        # Value boundary validations
        if goal.target_value <= 0:
            raise ValidationError(
                message="Goal target value must be greater than 0.",
                details=f"Target: {goal.target_value}"
            )

        if goal.category not in ('weight', 'steps', 'calories', 'water', 'sleep'):
            raise ValidationError(
                message="Goal category is invalid.",
                details=f"Category: {goal.category}"
            )

...
from app.utils.validators import validate_email, validate_user_profile


class UserService:
    """Orchestrates user management operations and enforces registration constraints."""

    def __init__(self, user_repo: UserRepository | None = None, profile_repo: UserProfileRepository | None = None):
    ...
    def register_user(self, user: User, profile: UserProfile) -> str:
        ...
        if existing_user:
            ...
            raise ValidationError(
                message="User registration failed: User already exists.", details=f"User ID: {user.user_id}"
            )
        ...
        if existing_by_email:
            ...
            raise ValidationError(
                message="User registration failed: Email address already registered.", details=f"Email: {user.email}"
            )
        ...
        try:
            ...
            logger.error(f"Failed to register user transaction: {e!s}")
            raise ServiceError("User registration transaction failed.", details=str(e))

    def get_user_and_profile(self, user_id: str) -> tuple[User, UserProfile] | None:
    ...
    def update_profile_metrics(self, user_id: str, weight_kg: float, height_cm: float):
        ...
        updates = {"weight_kg": weight_kg, "height_cm": height_cm}

...
class Settings:
    ...
    def __init__(self, env_file_name: str = ".env"):
        ...
        self.DEBUG_MODE: bool = (
            self.ENV in ("development", "testing") or
            self.LOG_LEVEL == "DEBUG" or
            os.getenv("DEBUG", env_vars.get("DEBUG", "False")).lower() in ("true", "1", "yes")
        )

        # System-wide Feature Flags (Production toggles)
        self.FEATURE_AI_COACH: bool = os.getenv("FEATURE_AI_COACH", env_vars.get("FEATURE_AI_COACH", "True")).lower() in ("true", "1", "yes")
        self.FEATURE_ANALYTICS: bool = os.getenv("FEATURE_ANALYTICS", env_vars.get("FEATURE_ANALYTICS", "True")).lower() in ("true", "1", "yes")
        self.FEATURE_REPORTS: bool = os.getenv("FEATURE_REPORTS", env_vars.get("FEATURE_REPORTS", "True")).lower() in ("true", "1", "yes")

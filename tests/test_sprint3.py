...
from app.models.domain import User, UserProfile, Exercise
...
class TestSprint3WorkoutEngine(unittest.TestCase):
    ...
    def test_exercise_logging_and_immutability(self):
        ...
        # Log Set 1 (Valid)
        self.workout_service.log_set(set_id="set-1", session_id="s-active", exercise_log_id="log-1", set_number=1, weight=100.0, reps=5, rpe=8.5)
        self.assertIsNotNone(self.set_repo.get_set("set-1"))

        # Validate duplicate set number blocking
        with self.assertRaises(ValidationError):
            self.workout_service.log_set(set_id="set-1-dup", session_id="s-active", exercise_log_id="log-1", set_number=1, weight=100.0, reps=5)

        # Validate boundary checks: weight < 0
        with self.assertRaises(ValidationError):
            self.workout_service.log_set(set_id="set-bad-w", session_id="s-active", exercise_log_id="log-1", set_number=2, weight=-10.0, reps=5)

        # Validate boundary checks: reps <= 0
        with self.assertRaises(ValidationError):
            self.workout_service.log_set(set_id="set-bad-r", session_id="s-active", exercise_log_id="log-1", set_number=2, weight=100.0, reps=0)

        # Validate boundary checks: RPE > 10.0
        with self.assertRaises(ValidationError):
            self.workout_service.log_set(set_id="set-bad-rpe", session_id="s-active", exercise_log_id="log-1", set_number=2, weight=100.0, reps=5, rpe=11.0)
        ...
        with self.assertRaises(ValidationError):
            self.workout_service.log_set(set_id="set-locked", session_id="s-active", exercise_log_id="log-1", set_number=3, weight=100.0, reps=5)
